...
def createBlocks(a1, bull):
    ...
    # AQUI
    phi0 = curvature_angle#-pi / 6
    phi1 = curvature_angle + inc #-pi / 6 + inc
    phi2 = curvature_angle + 2*inc #-pi / 6 + 2 * inc
    phi3 = curvature_angle + 3*inc #-pi / 6 + 3 * inc
...
def convert2cardiax(material_params, pvloop_params, baseName):
	...
	c.gmsh2xml(baseName + '.msh', baseName + '_cardiax.xml', material_params, 'bullseye/pvloop_data.txt')
...
def run(a1, c1, bull, material_params, pvloop_params, baseName):
    ...
    reduction = c1*sin( abs(curvature_angle) )/(1.0 + sin( abs(curvature_angle) ))
    c1 = c1 - reduction
